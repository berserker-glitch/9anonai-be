...
async function routeQuery(intent, query) {
    ...
    try {
        ...
        if (categoryFilter.length > 0) {
            ...
            results = await (0, retriever_1.searchLegalDocs)(query, 8, { categories: categoryFilter });
            // If domain search returns few results, fallback to general search
            if (results.length < 3) {
                const generalResults = await (0, retriever_1.searchLegalDocs)(query, 5);
            ...
            results = await (0, retriever_1.searchLegalDocs)(query, 8);
        }
