...
exports.sendWelcomeNewsletterEmail = sendWelcomeNewsletterEmail;
exports.sendReengagementEmail = sendReengagementEmail;
...
async function sendEmail(opts) {
    ...
    if (!client) {
        logger_1.logger.warn("[EMAIL] RESEND_API_KEY not set — skipping email send");
        return;
    }
    try {
        ...
        if (error) {
            logger_1.logger.error("[EMAIL] Resend error", { error });
        }
    }
    catch (err) {
        logger_1.logger.error("[EMAIL] Failed to send email", { error: err?.message });
    }
}
// ─── Welcome email (sent after registration) ─────────────────────────────────
async function sendWelcomeEmail(email, name, lang = "ar") {
...
async function sendWelcomeNewsletterEmail(email, lang = "ar") {
    const content = {
        ar: {
...
async function sendReengagementEmail(email, name, lang = "ar") {
    const displayName = name || email.split("@")[0];
    const content = {
        ar: {
