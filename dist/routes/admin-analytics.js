...
// ─────────────────────────────────────────────────────────────────────────────
router.get("/overview", auth_1.authenticate, auth_1.requireSuperAdmin, (0, error_handler_1.asyncHandler)(async (req, res) => {
    const range = req.query.range || "30d";
    const { startDate, prevStartDate } = parseDateRange(range);
    const adminId = req.userId;
    logger_1.logger.info(`[ANALYTICS] Overview requested by admin ${adminId}, range=${range}`);
    const now = new Date();
    const startOfToday = new Date(now.getFullYear(), now.getMonth(), now.getDate());
    const startOfWeek = new Date(now.getTime() - 7 * 24 * 60 * 60 * 1000);
    const startOfMonth = new Date(now.getTime() - 30 * 24 * 60 * 60 * 1000);
    // ── Totals (all time) ──
    const [totalUsers, totalConversations, totalMessages, totalDocuments, totalContractSessions, totalFileUploads,] = await Promise.all([
        prisma_1.prisma.user.count(),
        prisma_1.prisma.chat.count(),
        prisma_1.prisma.message.count(),
        prisma_1.prisma.generatedDocument.count(),
        prisma_1.prisma.contractSession.count(),
        prisma_1.prisma.userFile.count(),
    ]);
    // ── Period counts for growth ──
    const [currentPeriodUsers, previousPeriodUsers, currentPeriodConversations, previousPeriodConversations, currentPeriodMessages, previousPeriodMessages,] = await Promise.all([
        prisma_1.prisma.user.count({ where: { createdAt: { gte: startDate } } }),
        prisma_1.prisma.user.count({ where: { createdAt: { gte: prevStartDate, lt: startDate } } }),
        prisma_1.prisma.chat.count({ where: { createdAt: { gte: startDate } } }),
        prisma_1.prisma.chat.count({ where: { createdAt: { gte: prevStartDate, lt: startDate } } }),
        prisma_1.prisma.message.count({ where: { createdAt: { gte: startDate }, role: "user" } }),
        prisma_1.prisma.message.count({ where: { createdAt: { gte: prevStartDate, lt: startDate }, role: "user" } }),
    ]);
    // ── Active Users (DAU/WAU/MAU) using chat.updatedAt as proxy ──
    const [dauChats, wauChats, mauChats] = await Promise.all([
        prisma_1.prisma.chat.findMany({ where: { updatedAt: { gte: startOfToday } }, select: { userId: true }, distinct: ["userId"] }),
        prisma_1.prisma.chat.findMany({ where: { updatedAt: { gte: startOfWeek } }, select: { userId: true }, distinct: ["userId"] }),
        prisma_1.prisma.chat.findMany({ where: { updatedAt: { gte: startOfMonth } }, select: { userId: true }, distinct: ["userId"] }),
    ]);
    // ── Onboarding rate ──
    const onboardedCount = await prisma_1.prisma.user.count({ where: { isOnboarded: true } });
    ...
    const [totalLikes, totalDislikes] = await Promise.all([
        prisma_1.prisma.message.count({ where: { feedback: "like" } }),
        prisma_1.prisma.message.count({ where: { feedback: "dislike" } }),
    ]);
    const satisfactionRate = totalLikes + totalDislikes > 0 ? (totalLikes / (totalLikes + totalDislikes)) * 100 : 0;
    (0, logger_1.logDbOperation)("aggregate", "Analytics/Overview", true, `range=${range}`);
    res.json({
        totalUsers,
        totalConversations,
        totalMessages,
...
// ─────────────────────────────────────────────────────────────────────────────
router.get("/users/timeseries", auth_1.authenticate, auth_1.requireSuperAdmin, (0, error_handler_1.asyncHandler)(async (req, res) => {
    const range = req.query.range || "30d";
...
// ─────────────────────────────────────────────────────────────────────────────
router.get("/users/marketing-sources", auth_1.authenticate, auth_1.requireSuperAdmin, (0, error_handler_1.asyncHandler)(async (req, res) => {
    const range = req.query.range || "all";
    const { startDate } = parseDateRange(range);
    const grouped = await prisma_1.prisma.user.groupBy({
        by: ["marketingSource"],
        _count: { id: true },
...
// ─────────────────────────────────────────────────────────────────────────────
router.get("/engagement/timeseries", auth_1.authenticate, auth_1.requireSuperAdmin, (0, error_handler_1.asyncHandler)(async (req, res) => {
    const range = req.query.range || "30d";
    const { startDate, granularity } = parseDateRange(range);
    let msgRows;
    let chatRows;
...
// ─────────────────────────────────────────────────────────────────────────────
router.get("/engagement/activity-heatmap", auth_1.authenticate, auth_1.requireSuperAdmin, (0, error_handler_1.asyncHandler)(async (req, res) => {
    const range = req.query.range || "30d";
    const { startDate } = parseDateRange(range);
...
// ─────────────────────────────────────────────────────────────────────────────
router.get("/engagement/depth", auth_1.authenticate, auth_1.requireSuperAdmin, (0, error_handler_1.asyncHandler)(async (req, res) => {
    const range = req.query.range || "all";
    const { startDate } = parseDateRange(range);
    // Get per-user message counts (always all-time for depth analysis)
    const perUserRows = await prisma_1.prisma.$queryRaw `
        SELECT
            u.id AS userId,
...
// ─────────────────────────────────────────────────────────────────────────────
router.get("/engagement/power-users", auth_1.authenticate, auth_1.requireSuperAdmin, (0, error_handler_1.asyncHandler)(async (req, res) => {
...
// ─────────────────────────────────────────────────────────────────────────────
router.get("/content/overview", auth_1.authenticate, auth_1.requireSuperAdmin, (0, error_handler_1.asyncHandler)(async (req, res) => {
    const range = req.query.range || "30d";
    const { startDate, granularity } = parseDateRange(range);
    ...
    const [contractTotal, contractByType, contractByLanguage, contractByStatus] = await Promise.all([
        prisma_1.prisma.contractSession.count({ where: whereClause }),
        prisma_1.prisma.contractSession.groupBy({ by: ["contractType"], _count: { id: true }, where: whereClause, orderBy: { _count: { id: "desc" } } }),
        prisma_1.prisma.contractSession.groupBy({ by: ["language"], _count: { id: true }, where: whereClause }),
        prisma_1.prisma.contractSession.groupBy({ by: ["status"], _count: { id: true }, where: whereClause }),
    ]);
    let contractTimeseries;
    if (granularity === "day") {
        contractTimeseries = await prisma_1.prisma.$queryRaw `SELECT DATE_FORMAT(createdAt,'%Y-%m-%d') as date, COUNT(*) as count FROM ContractSession WHERE createdAt >= ${startDate} GROUP BY DATE_FORMAT(createdAt,'%Y-%m-%d') ORDER BY 1 ASC`;
    }
    else if (granularity === "week") {
        contractTimeseries = await prisma_1.prisma.$queryRaw `SELECT DATE_FORMAT(DATE_SUB(createdAt, INTERVAL WEEKDAY(createdAt) DAY),'%Y-%m-%d') as date, COUNT(*) as count FROM ContractSession WHERE createdAt >= ${startDate} GROUP BY DATE_FORMAT(DATE_SUB(createdAt, INTERVAL WEEKDAY(createdAt) DAY),'%Y-%m-%d') ORDER BY 1 ASC`;
    }
    else {
        contractTimeseries = await prisma_1.prisma.$queryRaw `SELECT DATE_FORMAT(createdAt,'%Y-%m') as date, COUNT(*) as count FROM ContractSession WHERE createdAt >= ${startDate} GROUP BY DATE_FORMAT(createdAt,'%Y-%m') ORDER BY 1 ASC`;
    }
    // ── Documents ──
    const [docTotal, docByType] = await Promise.all([
        prisma_1.prisma.generatedDocument.count({ where: whereClause }),
        prisma_1.prisma.generatedDocument.groupBy({ by: ["type"], _count: { id: true }, where: whereClause, orderBy: { _count: { id: "desc" } } }),
    ]);
    let docTimeseries;
    if (granularity === "day") {
        docTimeseries = await prisma_1.prisma.$queryRaw `SELECT DATE_FORMAT(createdAt,'%Y-%m-%d') as date, COUNT(*) as count FROM GeneratedDocument WHERE createdAt >= ${startDate} GROUP BY DATE_FORMAT(createdAt,'%Y-%m-%d') ORDER BY 1 ASC`;
    }
    else if (granularity === "week") {
        docTimeseries = await prisma_1.prisma.$queryRaw `SELECT DATE_FORMAT(DATE_SUB(createdAt, INTERVAL WEEKDAY(createdAt) DAY),'%Y-%m-%d') as date, COUNT(*) as count FROM GeneratedDocument WHERE createdAt >= ${startDate} GROUP BY DATE_FORMAT(DATE_SUB(createdAt, INTERVAL WEEKDAY(createdAt) DAY),'%Y-%m-%d') ORDER BY 1 ASC`;
    }
    else {
        docTimeseries = await prisma_1.prisma.$queryRaw `SELECT DATE_FORMAT(createdAt,'%Y-%m') as date, COUNT(*) as count FROM GeneratedDocument WHERE createdAt >= ${startDate} GROUP BY DATE_FORMAT(createdAt,'%Y-%m') ORDER BY 1 ASC`;
    }
    // ── Files ──
    const [fileTotal, fileByMimetype, fileSizeAgg] = await Promise.all([
        prisma_1.prisma.userFile.count({ where: whereClause }),
        prisma_1.prisma.userFile.groupBy({ by: ["mimetype"], _count: { id: true }, where: whereClause, orderBy: { _count: { id: "desc" } } }),
        prisma_1.prisma.userFile.aggregate({ _sum: { size: true }, where: whereClause }),
    ]);
    let fileTimeseries;
    if (granularity === "day") {
        fileTimeseries = await prisma_1.prisma.$queryRaw `SELECT DATE_FORMAT(createdAt,'%Y-%m-%d') as date, COUNT(*) as count FROM UserFile WHERE createdAt >= ${startDate} GROUP BY DATE_FORMAT(createdAt,'%Y-%m-%d') ORDER BY 1 ASC`;
    }
    else if (granularity === "week") {
        fileTimeseries = await prisma_1.prisma.$queryRaw `SELECT DATE_FORMAT(DATE_SUB(createdAt, INTERVAL WEEKDAY(createdAt) DAY),'%Y-%m-%d') as date, COUNT(*) as count FROM UserFile WHERE createdAt >= ${startDate} GROUP BY DATE_FORMAT(DATE_SUB(createdAt, INTERVAL WEEKDAY(createdAt) DAY),'%Y-%m-%d') ORDER BY 1 ASC`;
    }
    else {
        fileTimeseries = await prisma_1.prisma.$queryRaw `SELECT DATE_FORMAT(createdAt,'%Y-%m') as date, COUNT(*) as count FROM UserFile WHERE createdAt >= ${startDate} GROUP BY DATE_FORMAT(createdAt,'%Y-%m') ORDER BY 1 ASC`;
    }
    // ── Feedback ──
    const [feedbackLikes, feedbackDislikes] = await Promise.all([
        prisma_1.prisma.message.count({ where: { feedback: "like", createdAt: whereClause?.createdAt } }),
        prisma_1.prisma.message.count({ where: { feedback: "dislike", createdAt: whereClause?.createdAt } }),
    ]);
    let feedbackRows;
    if (granularity === "day") {
        feedbackRows = await prisma_1.prisma.$queryRaw `
    ...
    const [chatUsers, contractUsers, fileUsers, docUsers] = await Promise.all([
        prisma_1.prisma.chat.findMany({ select: { userId: true }, distinct: ["userId"] }),
        prisma_1.prisma.contractSession.findMany({ select: { userId: true }, distinct: ["userId"] }),
        prisma_1.prisma.userFile.findMany({ select: { userId: true }, distinct: ["userId"] }),
        prisma_1.prisma.generatedDocument.findMany({ select: { userId: true }, distinct: ["userId"] }),
    ]);
    const pct = (n) => totalUsers > 0 ? Math.round((n / totalUsers) * 1000) / 10 : 0;
    (0, logger_1.logDbOperation)("aggregate", "Analytics/Content", true, `range=${range}`);
    res.json({
        contracts: {
            total: contractTotal,
...
// ─────────────────────────────────────────────────────────────────────────────
router.get("/retention/cohorts", auth_1.authenticate, auth_1.requireSuperAdmin, (0, error_handler_1.asyncHandler)(async (req, res) => {
    const adminId = req.userId;
    logger_1.logger.info(`[ANALYTICS] Cohort retention requested by admin ${adminId}`);
    ...
    const activityRows = await prisma_1.prisma.$queryRaw `
    ...
    for (const row of activityRows) {
        if (!activityMap.has(row.userId))
            activityMap.set(row.userId, new Set());
...
// ─────────────────────────────────────────────────────────────────────────────
router.get("/retention/rates", auth_1.authenticate, auth_1.requireSuperAdmin, (0, error_handler_1.asyncHandler)(async (req, res) => {
    const now = new Date();
    // Day-N retention: of users who signed up at least N days ago,
    // how many sent a message on or after day N of their signup?
    const calcRetention = async (days) => {
        ...
        if (eligibleUsers.length === 0)
            return { eligible: 0, retained: 0, rate: 0 };
    ...
    const previouslyActive = await prisma_1.prisma.chat.findMany({
        where: { updatedAt: { gte: sixtyDaysAgo, lt: thirtyDaysAgo } },
        select: { userId: true },
        distinct: ["userId"],
    });
    const recentlyActive = await prisma_1.prisma.chat.findMany({
    ...
    const churnRate = previouslyActive.length > 0
        ...
        : 0;
    (0, logger_1.logDbOperation)("aggregate", "Analytics/Retention", true, "retention rates computed");
