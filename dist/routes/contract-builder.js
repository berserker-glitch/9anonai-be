...
const contract_builder_ai_1 = require("../services/contract-builder-ai");
...
const createSessionSchema = zod_1.z.object({
    contractType: zod_1.z.enum(["rental", "employment", "nda", "service", "sale", "custom"]),
    language: zod_1.z.enum(["ar", "fr", "en"]).default("fr"),
    title: zod_1.z.string().optional(),
});
/** Schema for streaming a message to an existing session */
const streamMessageSchema = zod_1.z.object({
...
router.post("/sessions", auth_1.authenticate, async (req, res) => {
    try {
        ...
        const session = await prisma_1.prisma.contractSession.create({
            data: {
        ...
        (0, logger_1.logDbOperation)("CREATE", "ContractSession", true, `id=${session.id}, type=${contractType}`);
        logger_1.logger.info(`[CONTRACT-ROUTES] New session created: ${session.id}`);
        ...
        logger_1.logger.error("[CONTRACT-ROUTES] Failed to create session:", error);
...
router.get("/sessions", auth_1.authenticate, async (req, res) => {
    try {
        ...
        const sessions = await prisma_1.prisma.contractSession.findMany({
            where: { userId },
            select: {
                id: true,
                title: true,
        ...
        logger_1.logger.error("[CONTRACT-ROUTES] Failed to list sessions:", error);
...
router.get("/sessions/:id", auth_1.authenticate, async (req, res) => {
    try {
        ...
        const session = await prisma_1.prisma.contractSession.findFirst({
            where: { id, userId },
            include: {
                messages: {
        ...
        logger_1.logger.error("[CONTRACT-ROUTES] Failed to get session:", error);
 ...
 */
router.post("/sessions/:id/stream", auth_1.authenticate, async (req, res) => {
    try {
        const authReq = req;
        const userId = authReq.userId;
        const { id } = req.params;
        ...
        await prisma_1.prisma.contractMessage.create({
            data: {
                role: "user",
                content: message,
                sessionId: id,
        ...
        logger_1.logger.info(`[CONTRACT-ROUTES] Streaming response for session ${id}`, {
        ...
        const stream = (0, contract_builder_ai_1.getContractStream)(message, sessionHistory, session.htmlContent, session.contractType, session.language, session.version);
        ...
        if (finalHtml !== session.htmlContent || newVersion !== session.version) {
            await prisma_1.prisma.contractSession.update({
                where: { id },
                data: {
            ...
            (0, logger_1.logDbOperation)("UPDATE", "ContractSession", true, `id=${id}, version=${newVersion}`);
        }
        res.end();
    }
    catch (error) {
        logger_1.logger.error("[CONTRACT-ROUTES] Stream error:", error);
 ...
 */
router.get("/sessions/:id/html", auth_1.authenticate, async (req, res) => {
    try {
        const authReq = req;
        const userId = authReq.userId;
        const { id } = req.params;
        ...
        logger_1.logger.error("[CONTRACT-ROUTES] Failed to get HTML:", error);
 ...
 */
router.post("/sessions/:id/export", auth_1.authenticate, async (req, res) => {
    try {
        const authReq = req;
        const userId = authReq.userId;
        const { id } = req.params;
        ...
        logger_1.logger.info(`[CONTRACT-ROUTES] Exporting PDF for session ${id}`);
        // Use the enhanced contract generator to produce PDF from raw HTML
        const document = await (0, contract_generator_1.generateFlexiblePDF)(userId, session.title, session.htmlContent, session.contractType, session.language);
        ...
        logger_1.logger.error("[CONTRACT-ROUTES] PDF export error:", error);
...
router.delete("/sessions/:id", auth_1.authenticate, async (req, res) => {
    try {
        ...
        await prisma_1.prisma.contractSession.delete({
            where: { id },
        });
        (0, logger_1.logDbOperation)("DELETE", "ContractSession", true, `id=${id}`);
        return res.json({ success: true });
    }
    catch (error) {
        logger_1.logger.error("[CONTRACT-ROUTES] Failed to delete session:", error);
