 ...
 */
function buildDraftingPrompt(legalContext: string, language: string): string {
...
async function retrieveLegalContext(
    ...
    try {
        ...
        if (domains.primary.length > 0) {
            logger.info(`[CONTRACT-AI] Primary RAG search for type="${contractType}"`, {
                categories: domains.primary,
            });
            const primaryResults = await searchLegalDocs(userQuery, 10, {
                categories: domains.primary,
            });
            addResults(primaryResults);
            logger.debug(`[CONTRACT-AI] Primary search returned ${primaryResults.length} results`);
        }

        // Secondary search: general obligations/contracts law (DOC)
        logger.info(`[CONTRACT-AI] Secondary RAG search (general obligations)`, {
            categories: domains.secondary,
        });
        const secondaryResults = await searchLegalDocs(userQuery, 8, {
            categories: domains.secondary,
        });
        addResults(secondaryResults);
        logger.debug(`[CONTRACT-AI] Secondary search returned ${secondaryResults.length} results`);
        ...
        logger.info(`[CONTRACT-AI] Tertiary RAG search (unfiltered, user query)`);
        const tertiaryResults = await searchLegalDocs(userQuery, 5);
        addResults(tertiaryResults);
        logger.debug(`[CONTRACT-AI] Tertiary search returned ${tertiaryResults.length} results`);
    ...
    logger.info(`[CONTRACT-AI] Total RAG results: ${allResults.length}, using top ${topResults.length}`);
    return topResults;
}
...
async function retrieveComplianceContext(
    ...
    try {
        ...
        logger.info(`[CONTRACT-AI] Compliance RAG: mandatory clauses search`);
        ...
        logger.info(`[CONTRACT-AI] Compliance RAG: prohibited terms search`);
        ...
        logger.info(`[CONTRACT-AI] Compliance RAG: formalities search`);
        const formalitiesResults = await searchLegalDocs(formalitiesQuery, 5);
        addResults(formalitiesResults);

    } catch (error) {
        logger.error("[CONTRACT-AI] Compliance RAG retrieval error:", error);
    }
...
function parseReviewResponse(text: string): {
    ...
    try {
        ...
        logger.warn("[CONTRACT-AI] Failed to parse review JSON, attempting manual extraction", error);
...
export async function* getContractStream(
    ...
    try {
        ...
        // ═══════════════════════════════════════════════════════════════
        logger.info(`[CONTRACT-AI] Starting contract stream`, {
            contractType,
            language,
        ...
        if (!chatResponse && !contractHtml) {
            logger.warn("[CONTRACT-AI] No XML tags found in response, using full text as chat");
        ...
        if (!contractHtml) {
            logger.info("[CONTRACT-AI] No contract HTML in response (chat-only reply)");
            yield { type: "done" };
            return;
        }
        ...
        // ═══════════════════════════════════════════════════════════════
        logger.info("[CONTRACT-AI] Starting legal compliance review (Phase 2)");
        ...
        logger.info(`[CONTRACT-AI] Compliance RAG: ${complianceSources.length} references found`);
        ...
        logger.info(`[CONTRACT-AI] Review complete: ${reviewResult.issues.length} issues found`, {
