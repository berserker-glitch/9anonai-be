...
import { buildContext } from "./query-router";
...
// ─────────────────────────────────────────────────────────────────────────────
// Types (unchanged — keeps API compatibility with the route handler)
// ─────────────────────────────────────────────────────────────────────────────

/** A single issue found during legal compliance review */
...
/** Result of the legal compliance review */
export interface ReviewResult {
    issues: ReviewIssue[];
    summary: string;
}

/** Events streamed to the frontend via SSE */
export type ContractStreamEvent =
    | { type: "step"; content: string }
    | { type: "token"; content: string }
    | { type: "html_update"; html: string; version: number }
    | { type: "review_result"; issues: ReviewIssue[]; summary: string }
    | { type: "sources"; sources: LegalDocument[] }
...
const CONTRACT_TYPE_DOMAINS: Record<string, { primary: string[]; secondary: string[] }> = {
    rental: {
 ...
 */
function buildReviewPrompt(complianceContext: string, language: string): string {
    const langInstruction = getLanguageInstruction(language);
...
async function retrieveLegalContext(
    ...
    const addResults = (results: LegalDocument[]) => {
    ...
    try {
        ...
        if (domains.primary.length > 0) {
            logger.info(`[CONTRACT-AI] Primary RAG: type="${contractType}"`, { categories: domains.primary });
            const primaryResults = await searchLegalDocs(userQuery, 10, { categories: domains.primary });
            addResults(primaryResults);
            logger.debug(`[CONTRACT-AI] Primary: ${primaryResults.length} results`);
        }

        // Secondary: general obligations/contracts law
        logger.info(`[CONTRACT-AI] Secondary RAG (general obligations)`);
        const secondaryResults = await searchLegalDocs(userQuery, 8, { categories: domains.secondary });
        addResults(secondaryResults);
        logger.debug(`[CONTRACT-AI] Secondary: ${secondaryResults.length} results`);

        // Tertiary: unfiltered search for edge cases
        logger.info(`[CONTRACT-AI] Tertiary RAG (unfiltered)`);
        const tertiaryResults = await searchLegalDocs(userQuery, 5);
        addResults(tertiaryResults);
        logger.debug(`[CONTRACT-AI] Tertiary: ${tertiaryResults.length} results`);

    } catch (error) {
        logger.error("[CONTRACT-AI] RAG retrieval error:", error);
    }

    allResults.sort((a, b) => (b.score || 0) - (a.score || 0));
    const topResults = allResults.slice(0, 15);
    logger.info(`[CONTRACT-AI] Total RAG: ${allResults.length}, using top ${topResults.length}`);
    return topResults;
}
 ...
 */
async function retrieveComplianceContext(
    contractType: string,
    contractHtml: string
): Promise<LegalDocument[]> {
    const allResults: LegalDocument[] = [];
    ...
    try {
        ...
        logger.info(`[CONTRACT-AI] Compliance RAG: mandatory clauses`);
        const mandatoryResults = await searchLegalDocs(mandatoryQuery, 8);
        addResults(mandatoryResults);

        const prohibitedQuery = `clauses abusives interdites contrat ${contractType} maroc`;
        logger.info(`[CONTRACT-AI] Compliance RAG: prohibited terms`);
        const prohibitedResults = await searchLegalDocs(prohibitedQuery, 5);
        addResults(prohibitedResults);

        const formalitiesQuery = `formalités légales obligatoires contrat ${contractType} maroc`;
        logger.info(`[CONTRACT-AI] Compliance RAG: formalities`);
        const formalitiesResults = await searchLegalDocs(formalitiesQuery, 5);
        addResults(formalitiesResults);

    } catch (error) {
        logger.error("[CONTRACT-AI] Compliance RAG error:", error);
    }
 ...
 */
function stripLegacyXmlTags(text: string): string {
    return text
 ...
 */
function extractContractHtml(text: string): string {
 ...
 */
function extractChatMessage(text: string): string {
 ...
 */
function parseReviewResponse(text: string): { issues: ReviewIssue[]; summary: string } {
    try {
        ...
        logger.warn("[CONTRACT-AI] Failed to parse review JSON, returning empty result");
        return {
            issues: [],
 ...
 */
export async function* getContractStream(
    userMessage: string,
...
): AsyncGenerator<ContractStreamEvent, void, unknown> {
    try {
        // ═══════════════════════════════════════════════════════════════
        // PHASE 1: RAG-Powered Drafting
        // ═══════════════════════════════════════════════════════════════
        logger.info("[CONTRACT-AI] Starting pipeline", {
            contractType,
            language,
        ...
        const legalContext = buildContext(legalSources);
        ...
        for await (const chunk of stream) {
            ...
            if (tagPos === -1) {
                ...
                if (lastStreamedPos < tagPos) {
                    const remaining = stripLegacyXmlTags(
                        fullResponse.slice(lastStreamedPos, tagPos)
                    );
                    if (remaining.trim()) {
        ...
        if (!contractHtml) {
            logger.info("[CONTRACT-AI] Chat-only response (no contract HTML generated)");
            yield { type: "done" };
            return;
        }
        ...
        // ═══════════════════════════════════════════════════════════════
        logger.info("[CONTRACT-AI] Starting compliance review (Phase 2)");

        try {
            const complianceSources = await retrieveComplianceContext(contractType, contractHtml);
            const complianceContext = buildContext(complianceSources);
            logger.info(`[CONTRACT-AI] Compliance RAG: ${complianceSources.length} references`);
            ...
            logger.info(`[CONTRACT-AI] Review complete: ${review.issues.length} issues`, {
            ...
            logger.error("[CONTRACT-AI] Review failed (non-blocking):", reviewError);
            yield { type: "step", content: "Contract ready — review could not be completed." };
        }

        yield { type: "done" };
    } catch (error) {
        logger.error("[CONTRACT-AI] Pipeline error:", error);
